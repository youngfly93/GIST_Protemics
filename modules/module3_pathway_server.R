...
module3_pathway_server <- function(id) {
  moduleServer(id, function(input, output, session) {
    ...
    # 信息框输出
    output$n_positive_box <- renderInfoBox({
      results <- analysis_results()
      n_pos <- if (!is.null(results)) length(results$pathway$positive) else 0
      
      infoBox(
        title = "Positive Correlated",
        value = n_pos,
        subtitle = "proteins",
        icon = icon("arrow-up"),
        color = "success",
    ...
    output$n_negative_box <- renderInfoBox({
      results <- analysis_results()
      n_neg <- if (!is.null(results)) length(results$pathway$negative) else 0
      
      infoBox(
        title = "Negative Correlated",
        value = n_neg,
        subtitle = "proteins",
    ...
    output$correlation_plot <- renderPlot({
      ...
      plot_data <- data.frame(
        Type = c(rep("Positive", length(results$pathway$positive)),
                 rep("Negative", length(results$pathway$negative))),
        Index = c(seq_along(results$pathway$positive),
                  seq_along(results$pathway$negative))
      )
      
      ggplot(plot_data, aes(x = Type, fill = Type)) +
        geom_bar(stat = "count") +
        scale_fill_manual(values = c("Positive" = "#28a745", "Negative" = "#dc3545")) +
        labs(title = paste("Correlated Proteins for", input$protein_id),
             x = "Correlation Type",
    ...
    output$all_proteins_table <- DT::renderDataTable({
      results <- analysis_results()
      req(results)
